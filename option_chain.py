...
def option_sentiment(
    ...
    return "NEUTRALimport streamlit as st
import pandas as pd
from kiteconnect import KiteConnect
...
def get_live_option_chain(spot_price, strikes_each_side=10):
    ...
    atm = round(float(spot_price) / 50) * 50
...
def calculate_max_pain(option_data):
    ...
    for test_strike in strikes:
        ...
        pain[test_strike] = call_pain + put_pain

    return min(pain, key=pain.get)


def calculate_snapshot_oi_change(current_data):
    ...
    st.session_state["previous_option_oi"] = dict(
        zip(
            data["symbol"],
            data["oi"].fillna(0)
        )
